---
title: "Chapter 0: What is a Statistical Model<br><span style='font-size:70%'>Part 2</span>"
subtitle: "Statistical Modeling"
author: "MATH 360"
format:
  revealjs:
    chalkboard: true
    theme: [default, custom.scss]
    toc: false
    toc-depth: 1
    reveal-options:
      slideNumber: true
    html-math-method: mathjax
    incremental: false
    transition: fade
    preview-links: auto
    notes: true
title-slide-attributes:
  data-background-color: "#EA6820"
from: markdown+emoji
execute:
  echo: true
  cache: false
editor:
  markdown:
    wrap: 72
---

```{r packages, echo=FALSE, message=FALSE, warning=FALSE}
library(tidyverse)
library(rvest)
library(mosaic)
library(Stat2Data)
data("WeightLossIncentive4")
```

## Topics :pushpin:

::: incremental
-   Statistical model

-   4-step process

-   Case study

-   R Code
:::

## Statistical modeling

-   A general statistical model:

$$
Y = f(X) + \epsilon
$$

-   Goal: Find a model for a relationship between a **response
    variable** ($Y$) and one or more **explanatory/predictor** variables
    $(X_1, X_2, \ldots, X_k)$.

##  {background-image="figures/nyt_model.png" background-size="contain" background-position="center"}

## Four-step Modeling Process

::: {.incremental .highlight-last}
1.  **Choose**: Pick a form for the model.

2.  **Fit**: Estimate any parameters.

3.  **Assess**: Is the model adequate? Could it be simpler? Are
    conditions met?

4.  **Use**: Answer the question of interest
:::

## {.smaller}
### Study: Financial incentives for weight loss

::::: columns
::: {.column width="50%"}
![](figures/wt_loss_title.png){fig-align="left"}

-   **Research question**: Do financial incentives help people lose
    weight more successfully?
:::

::: {.column .fragment width="50%"}
-   **Participants**: healthy adults 30-70 with BMI of 30-40

-   **Treatment groups**: Randomly assigned to treatment groups
    (*Group*):

    -   Financial incentives vs

    -   Control (monthly weigh-ins, no financial incentive)

-   **Response**: *WeightLoss* = Before - After

    -   Positive $\Rightarrow$ weight loss

-   R Dataset: **WeightLossIncentive4**
:::
:::::

------------------------------------------------------------------------

### Study: Financial incentives for weight loss

#### The data

::: panel-tabset
## Access and Group Size

```{r fi1}
# Load data set
data("WeightLossIncentive4")
tally(~Group, data = WeightLossIncentive4)
```

## Control

```{r fi2}
filter(WeightLossIncentive4, Group == "Control")
```

## Incentive

```{r fi3}
filter(WeightLossIncentive4, Group == "Incentive")
```
:::

------------------------------------------------------------------------

### Describing the Data Graphically and Nummerically

::: panel-tabset
## R Code

```{r figraph1, eval = FALSE}
# coord_flip makes dotplot vertical
gf_dotplot(~WeightLoss | Group, data = WeightLossIncentive4) + 
  coord_flip()
gf_boxplot(WeightLoss ~ Group, data = WeightLossIncentive4)
# summary statistics by group
favstats(WeightLoss ~ Group, data = WeightLossIncentive4)
```

## Dotplot

```{r figraph2, echo = FALSE}
gf_dotplot(~WeightLoss | Group, data = WeightLossIncentive4) + 
  coord_flip()
```

## Boxplot

```{r figraph3, echo = FALSE}
gf_boxplot(WeightLoss ~ Group, data = WeightLossIncentive4)
```

## Summary Statistics

```{r fi_stats, echo = FALSE}
favstats(WeightLoss ~ Group, data = WeightLossIncentive4)
```
:::

## Step 1: Choose a Model

::::: columns
::: {.column width="50%"}
### Characteristics

Consider a model suggested by the data:

-   Different means for each treatment group ( $\mu_i$ )

-   Common variability ( $\sigma$ )

-   Normally distributed (symmetric) errors ( $\epsilon$ )
:::

::: {.column .fragment width="50%"}
### Formally stated:

$$
Y = \mu_i + \epsilon \mbox{ where } \epsilon \sim N(0, \sigma)
$$

### By group:

-   Control Group:\
    $\displaystyle Y = \mu_1 + \epsilon \text{ so } Y \sim N(\mu_1, \sigma_1)$
-   Incentives Group:\
    $\displaystyle Y = \mu_2 + \epsilon \text{ so } Y \sim N(\mu_2, \sigma)$
:::
:::::

## Step 2: Fit the Model {.smaller}

### Process

-   Depends on study design, types of variables, and proposed model.

::: fragment
### Financial incentives model

-   Compute sample estimates of means $\bar y_i$ and standard deviations
    $s_i$

-   Fitted values differ by treatment group:

$\qquad \hat y = \bar y_i$

-   **Note**: No error term in the *fitted model*

-   **Interpretation**: Predicted weight loss for a member of group $i$.
:::

## Step 3: Assess the Model

-   Determine whether the conditions of the model hold

    -   Look at residual $y-\hat y_i$ which is just $y-\bar y_i$ for the 2-means model.

-   Consider whether a different (simpler) model might fit.

## Check conditions on residuals

-   Use R function `lm()` (for linear model) to fit the two-mean model (see code example)

    + $\Rightarrow$ The `lm()` function fits a model assuming constant standard deviations.

-   Verify fitted values

-   Plot residuals to check conditions

## R code and output for checking conditions on residuals {.smaller}

::: panel-tabset
## R Code

```{r fimod1, eval = FALSE}
# fits two-means model with equal standard deviations
wl_mod <- lm(WeightLoss ~ Group, data = WeightLossIncentive4)

# adds fitted and residual values to the dataset
WeightLossIncentive4 <- WeightLossIncentive4 |>  
  mutate(yhat = fitted(wl_mod), resid = resid(wl_mod))

# check the values
head(filter(WeightLossIncentive4, Group == "Control"))
head(filter(WeightLossIncentive4, Group == "Incentive"))

# residual dotplots 
gf_dotplot(~resid | Group, data = WeightLossIncentive4) + 
  coord_flip()

# residual normal quantile plot
gf_qq(~ resid, data = WeightLossIncentive4) |> 
  gf_qqline()
```


## Fitted values and residuals

```{r fimod2, echo = FALSE}
# fits two-means model with equal standard deviations
wl_mod <- lm(WeightLoss ~ Group, data = WeightLossIncentive4)

# adds fitted and residual values to the dataset
WeightLossIncentive4 <- WeightLossIncentive4 %>% 
  mutate(yhat = fitted(wl_mod), resid = resid(wl_mod))

# check the values
head(filter(WeightLossIncentive4, Group == "Control"))
head(filter(WeightLossIncentive4, Group == "Incentive"))
```

## Dotplot

```{r fimod3, echo = FALSE}
gf_dotplot(~resid | Group, data = WeightLossIncentive4) + 
  coord_flip()
```

## Normal quantile plot

```{r fimod4, echo = FALSE}
gf_qq(~ resid, data = WeightLossIncentive4) |>  gf_qqline()
```
:::


## Comments on Residual checks

* Residuals have simiiar standard deviations
* Residuals appear reasonably normally distributed


## Test versus a simpler model

Carry out a two-sample $t$-test for financial incentives data to see if there is sufficient evidence to use a two means rather than one mean model:

$$
H_0: \mu_1 = \mu_2  \quad\Rightarrow \quad Y = \mu + \epsilon \quad \mbox{ (one mean model)} 
$$

versus

$$
H_a: \mu_1 \neq \mu_2 \quad \Rightarrow \quad Y = \mu_i + \epsilon \quad \mbox{ (two means model)}
$$

------------------------------------------------------------------------

### Two-sample t-test for financial incentives data

**Which model is preferred?**

```{r}
#| output-location: fragment
t.test(WeightLoss ~ Group, data = WeightLossIncentive4, var.equal = TRUE)
```

::: aside
Note use of `var.equal` here to specify equal standard deviations.
:::



## Step 4: Use the Model

-   The low $p$-value from the test points us to prefer **two means model** ($H_a$)

-   Estimate treatment effect

-   Fund more studies

-   Health care policy implications


## The p-value controversy {background-image="figures/pvalue_controversy.png" background-size="contain" background-position="center"}

## The Trouble with the p-value

::: incremental
-   The $p$-value tells the **strength of evidence** of an "effect"

-   The $p$-value tells nothing about the **size** or **practical
    importance** of an "effect"
:::

## Step 4: Use: Answer the question of interest

::: incremental
-   Confidence interval: estimates the **size** of the effect

-   Scope of inference

    -   Can causal inference be concluded?
    -   To what population(s) can we generalize?
    -   Is the **effect size** large enough to be *important*?
:::

## Effect size {.smaller}

::: incremental
-   **Effect size**: "an often standardized measure of the magnitude of
    a certain phenomenon of scientific interest."[^1]

-   **Two quantitative variables**

    -   Correlation coefficient $r$

    -   Regression slope (or Beta coefficients or Cohen's $f^2$)

-   **Comparing populations**

    -   Standardized mean difference $\theta$ $$
        \theta = \frac{\mu_1-\mu_2}{\sigma}
        $$ where $\sigma$ is the standard deviation of one (or both) of
        the populations being compared.

:::

##
### Estimated Effect Size for Difference in Means {.smaller}

$$
\mbox{effect size} = \frac{\mbox{Difference in sample means}}{\mbox{Estimated Standard Deviation}} = \frac{\bar y_1-\bar y_2}{s_p}
$$

::: {.fragment}
Where $s_p$ is "pooled" standard deviation defined as: 
$$
s_p=\sqrt{\frac{(n_1-1)s_1^2 + (n_2-1)s_2^2}{n_1+n_2-2}}
$$
:::
:::{.fragment}
-   Alternatively, for financial incentives data, we might estimate
    $\sigma$ by $s$, the standard deviation of weight losses for control
    ("untreated") individuals

:::

[^1]: Schafer T and Schwarz MA (2019) The Meaningfulness of Effect Sizes
    in Psychological Research: Differences Between Sub-Disciplines and
    the Impact of Potential Biases. Front. Psychol. 10:813. doi:
    10.3389/fpsyg.2019.00813


##
### Example: Effect Size from Study of Financial Incentives for Weight Loss {.smaller}


```{r}
favstats(WeightLoss ~ Group, data = WeightLossIncentive4)
```

:::{.fragment}
* Using $s_p$ in the denominator:
$\mbox{effect size} = \frac{|\bar y_1 - \bar y_2|}{s_p} =  \frac{|15.68 - 3.92|}{9.25} = 1.27$
:::

:::{.fragment}
* Using $s_C$ (control sd) in the denominator:
$\mbox{effect size} = \frac{|\bar y_1 - \bar y_2|}{s_c} =  \frac{|15.68 - 3.92|}{9.11} = 1.29$
:::

## How to Assess Effect Sizes?

- General guidelines for effect sizes

  - Small: 0.2
  - Medium: 0.5
  - Large: 1.0
  
:::{.fragment}
- But these are no substitute for judgement in context.

  - How does difference in average weight loss of 11.76 pounds compare with starting weights?
  
  - Was weight loss sustained?
:::



## Summary :memo: {.smaller}


* The **C–F–A–U process** guides modeling: choose a model, fit it, assess assumptions, and use the model to answer the research question.

* A **p-value** provides evidence of a difference but does not tell us the **magnitude** or **practical importance** of the effect.

* **Effect size** quantifies how large the difference is; in the weight-loss study, the standardized mean difference was **very large**.

* Interpretation always requires **context**: consider generalizability, causality, and whether the effect is meaningful in real-world terms.


